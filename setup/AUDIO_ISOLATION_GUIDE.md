...
- `pihome-user` is in `gpio`, `video`, `render`, `input` groups → can access GPIO pins, GPU, and touchscreen
...
- Add `pihome-user` to `gpio`, `video`, `render`, `input` groups (hardware access)
