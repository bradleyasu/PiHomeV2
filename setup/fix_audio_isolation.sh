#!/bin/bash
# Emergency fix script if pihome can access hw:1,0
...
echo "[1/7] Checking current pihome group membership..."
PIHOME_GROUPS=$(groups pihome 2>/dev/null || echo "")
echo "Current groups: $PIHOME_GROUPS"

if echo "$PIHOME_GROUPS" | grep -q audio; then
    echo "⚠ WARNING: pihome is in audio group (THIS IS THE PROBLEM)"
    echo "[2/7] Removing pihome from audio group..."
    gpasswd -d pihome audio
    echo "✓ Removed pihome from audio group"
else
    echo "[2/7] pihome is NOT in audio group (correct)"
fi

echo ""
echo "[3/7] Setting pihome user's primary group to pihome..."
usermod -g pihome pihome
echo "✓ Primary group set to pihome"
...
if sudo -u pihome speaker-test -D hw:0,0 -c 2 -t sine -l 1 > /dev/null 2>&1; then
    ...
    echo "   Check if pihome group has access to hw:0,0"
fi
...
echo ""
echo "Current status:"
echo "  - pihome groups: $(groups pihome)"
